...
def get_lr_scheduler(optimizer, lr_scheduler_type='fixed', **kwargs):
    ...
    '''
    if lr_scheduler_type in ['fixed','fixedsmall']:
...
class BNDownBlock(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, ksize=3, padding=1, batch_norm=True):
...
class BNUpBlock(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, which_conv='conv', padding=1, batch_norm=True):
...
class output_block(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, which_conv='conv', which_activation='relu', batch_norm=False, padding=1):
...
class SNDownBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, ksize=3, padding=1):
...
class unet_translator(nn.Module):
    """Defines the translator model for MVS"""

    def __init__(self, n_output_channels, last_activation='relu', flag_asymmetric=True, flag_multiscale=True, 
                 n_input_channels=3, n_filter=32, depth=6, which_decoder='conv', encoder_padding=1, decoder_padding=1):
        """
        Parameters:
        ...
        # if multiscale and i even (except for i=0), then add 3 he channels 
            self.encoder.append(
                BNDownBlock(encoder_sizes[i] + n_input_channels * self.flag_multiscale * ((i%2)==0) * (i!=0), 
                            encoder_sizes[i+1], padding=encoder_padding)
            )
        
        # ----- bottleneck/center block -----
        self.center_block.append(BNDownBlock(encoder_sizes[-1], encoder_sizes[-1], padding=encoder_padding)) 
        self.center_block.append(BNUpBlock(encoder_sizes[-1], encoder_sizes[-1], which_conv=which_decoder, padding=decoder_padding))
        
        # ----- decoder -----
        for i in range(depth-2):
            self.decoder.append(
                BNUpBlock(decoder_sizes[i]*2, decoder_sizes[i+1], which_conv=which_decoder, padding=decoder_padding)
            )
        ...
        self.decoder.append(output_block(decoder_sizes[i+1]*2, decoder_sizes[i+2], which_conv=which_decoder, 
                                         which_activation=self.last_activation, padding=decoder_padding))
...
class Discriminator(torch.nn.Module):
    """ defines the Discriminator model for MVS """
    
    def __init__(self, n_output_channels, flag_asymmetric=True, flag_multiscale=True, n_input_channels=3, n_filter=32, depth=6):
        """
        Parameters:
            n_output_channels (int) -- desired number of channels in output, here for IMC 
            flag_asymmetric (bool) -- if input higher resolution than output 
        ...
        if self.flag_asymmetric:
            ...
            for i in range(2): 
                self.asymmetric_block.append(
                    SNDownBlock(asymmetric_sizes[i], asymmetric_sizes[i+1])
                )
