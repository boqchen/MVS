...
def standardize_imc_array(imc_array: np.array, protein_subset=PROTEIN_LIST):
    if protein_subset != PROTEIN_LIST:
        idx_list = [protein2index[prot_name] for prot_name in protein_subset]
        return (imc_array - (np.array(EXPRS_AVG)[np.newaxis, np.newaxis, :])[:, :, idx_list]) / (np.array(EXPRS_STDEV)[np.newaxis, np.newaxis, :])[:, :, idx_list]
    else:
        return (imc_array - np.array(EXPRS_AVG)[np.newaxis, np.newaxis, :]) / np.array(EXPRS_STDEV)[np.newaxis, np.newaxis, :]


def de_standardize_imc_array(imc_array: np.array, protein_subset=PROTEIN_LIST):
    if protein_subset != PROTEIN_LIST:
        idx_list = [protein2index[prot_name] for prot_name in protein_subset]
        return (imc_array * (np.array(EXPRS_STDEV)[np.newaxis, np.newaxis, :])[:, :, idx_list]) + (np.array(EXPRS_AVG)[np.newaxis, np.newaxis, :])[:, :, idx_list]
    else:
        return (imc_array * np.array(EXPRS_STDEV)[np.newaxis, np.newaxis, :]) + np.array(EXPRS_AVG)[np.newaxis, np.newaxis, :]
...
def get_manually_aligned_he_region(align_result: dict, level: int, verbose=0):
    ...
    # @TODO why does x have to be negative, and y positive ? Might be because of flipping ?
    ...
    # @TODO think about the max margin that we would need, I guess for 45 degrees
    margin_factor = 0.5
...
def do_test():
    ''' Simple test of the functions in this file.
    '''
    # base paths 
    imc_roi_basepath = '/cluster/work/tumorp/data_repository/study/TP-M1-USZ-011/MATIWAQ/imc/raw/pass_1'
    he_image_path = '/cluster/work/tumorp/data_repository/study/TP-M1-USZ-011/MATIWAQ/digpath_zurich/raw/pass_1/MATIWAQ-P__DP-W2.krasv.HE_1.tif'
    roi_locations_path = '/cluster/work/tumorp/data-drop/imc_he_rois/study/melanoma/MATIWAQ/MATIWAQ-P__DP-W2.krasv.HE_1.tif.json'

    assert os.path.exists(imc_roi_basepath) and os.path.exists(he_image_path) and os.path.exists(roi_locations_path)

    # test coordinate/name extraction
    rois_name_to_coords = get_rois_names_and_coords(roi_locations_path)
    print(rois_name_to_coords)

    # get corresponding h&e for each IMC roi locations 
    slide = openslide.open_slide(he_image_path)

    print(slide)
    print('downsamples: ', slide.level_downsamples) 
    print('dimensions for downsamples:', slide.level_dimensions) 
    print('Resolution um/pixel: ', slide.properties['openslide.mpp-x'])

    resolution_imc = IMC_RESOLUTION
    resolution_he = get_he_res(slide)
    level_in_he = math.ceil(math.log2(resolution_imc / resolution_he))

    # test he region extraction
    for roi_name in list(rois_name_to_coords.keys()): 
        roi_coords = rois_name_to_coords[roi_name]
        he_roi = get_he_region(slide, level_in_he, roi_coords)
        print(he_roi.shape)

    # test imc raw text file reading:
    print(read_raw_protein_txt("/cluster/work/tumorp/data_repository/study/TP-M2-USZ-131/MYHAJIS/imc/raw/pass_1/MYHAJIS-I_M4.1-v1.1-r1v1.1__F2_10.txt").shape)
