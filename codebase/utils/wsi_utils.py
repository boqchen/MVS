...
class BagOfTiles(Dataset):
    def __init__(self, wsi, tiles, normalizer=None):
        ...
        self.normalize = transforms.Normalize(
            mean = [0.662, 0.352, 0.688],
            std=[0.176, 0.180, 0.147]
        )
...
def get_wsi_inference(sample, wsi, seg_level, chunk_size, batch_size, n_proteins, loader_kwargs, dev0, network, save_path_imgs, normalizer): 
    ...
    imc_pred_level6 = np.zeros((wsi.level_dimensions[0][1]//2**6,wsi.level_dimensions[0][0]//2**6, n_proteins), dtype=np.float32)
    imc_pred_level4 = np.zeros((wsi.level_dimensions[0][1]//2**4,wsi.level_dimensions[0][0]//2**4, n_proteins), dtype=np.float32)
    imc_pred_level2 = np.zeros((wsi.level_dimensions[0][1]//2**2,wsi.level_dimensions[0][0]//2**2, n_proteins), dtype=np.float32)
    
    print(imc_pred_level6.shape, imc_pred_level4.shape, imc_pred_level2.shape)
    ...
    for batch_id, (batch, coord) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            # print('imc_batch: ', len(imc_batch), imc_batch[0].shape, imc_batch[1].shape, imc_batch[2].shape)
...
# Note: scaling using cohort stasts takes too long here therefore do as post-processing or later try without apply along axis 
#     def minmax_scaling(x, max_cohort):
#         return (x)/max_cohort

#     # scaling using cohorts quantiles 
#     imc_pred_level6 = np.apply_along_axis(minmax_scaling, 2, imc_pred_level6, max_cohort=cohort_quantiles_protein_subset)
#     imc_pred_level4 = np.apply_along_axis(minmax_scaling, 2, imc_pred_level4, max_cohort=cohort_quantiles_protein_subset)
#     imc_pred_level2 = np.apply_along_axis(minmax_scaling, 2, imc_pred_level2, max_cohort=cohort_quantiles_protein_subset)
    
#     np.save(path_level6.joinpath(sample + '_scaled.npy'), imc_pred_level6)    
#     np.save(path_level4.joinpath(sample + '_scaled.npy'), imc_pred_level4)    
#     np.save(path_level2.joinpath(sample + '_scaled.npy'), imc_pred_level2) 