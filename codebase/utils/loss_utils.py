...
def get_r1(discriminator, he, imc_list, gamma_0, lazy_c):
    ...
    # # Make sure requires_grad is True for all tensors in imc_list
    # imc_list = [imc.detach().requires_grad_(True) for imc in imc_list]

    # # Compute the score_maps
    # score_maps = discriminator(he, imc_list)

    # # For each score_map
    # for i, score_map in enumerate(score_maps):
    #     # Compute the sum of the score_map. This is necessary because the score_map is usually
    #     # multiple values (one for each example in the batch), and we need a single value to compute
    #     # the gradient.
    #     score_map_sum = score_map.sum()
        
    #     imc = imc_list[len(imc_list) - i -1]

    #     # Compute the gradient of the score_map_sum with respect to each tensor in imc_list
    #     grads = torch.autograd.grad(outputs=[score_map.sum()], inputs=[imc], create_graph=True, only_inputs=True)
    #     print(grads)
        

        # # For each tensor and its corresponding gradient
        # for j, (imc, grad) in enumerate(zip(imc_list, grads)):
        #     # Check if the gradient is None
        #     if grad is None:
        #         print(f'Tensor at index {j} is not used in the computation of score_map at index {i}')
        #     else:
        #         print(f'Tensor at index {j} is used in the computation of score_map at index {i}')
