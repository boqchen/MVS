...
# kid_list = []
# protein_list = []
fid_dict = {protein: [] for protein in protein_subset}
kid_dict = {protein: [] for protein in protein_subset}
...
for protein in protein_subset:
    ...
    eval_ds = Eval_dataset(imc_pred_paths, imc_roi_storage, channel_list=protein2index[protein])
    ...
    for i, batch in enumerate(evalloader):
        ...
        imc_pred = batch['imc_pred'][:, protein_subset.index(protein), :, :].unsqueeze(dim=1).to(dev0) # [B, C, H, W] --> [B, 1, H, W]
        imc_gt = batch['imc_gt'].to(dev0) # [B, 1, H, W]
        
        imc_pred = imc_pred.repeat(1, 3, 1, 1) # [B, 1, H, W] --> [B, 3, H, W]
        imc_gt = imc_gt.repeat(1, 3, 1, 1) # [B, 1, H, W] --> [B, 3, H, W]
        ...
        fid_dict[protein].append(round(fid_score.item(), 4))
        kid_dict[protein].append(round(kid_score.item(), 4))
...
agg_fid = {protein: sum(values)/len(values) if values else 0 for protein, values in fid_dict.items()}
agg_kid = {protein: sum(values)/len(values) if values else 0 for protein, values in kid_dict.items()}
df_fid = pd.DataFrame(list(agg_fid.items()), columns=['Protein', 'Aggragated FID'])
df_kid = pd.DataFrame(list(agg_kid.items()), columns=['Protein', 'Aggragated KID'])
